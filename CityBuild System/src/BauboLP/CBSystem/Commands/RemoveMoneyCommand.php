...
class RemoveMoneyCommand extends Command
{
    ...
    {
        if(!$sender->hasPermission("cb.remove.money")) {
            $sender->sendMessage(CBSystem::Prefix.CBSystem::getPlugin()->getTextContainer("Player-has-no-perms"));
            return;
        }

        if(!isset($args[0]) or !isset($args[1])) {
            $sender->sendMessage(CBSystem::Prefix."/removemoney [PLAYER] [AMOUNT]");
            return;
        }
        ...
        CBSystem::getPlugin()->removeMoney($player, $args[1]);
        $sender->sendMessage(CBSystem::Prefix.CBSystem::getPlugin()->getTextContainer("money-was-removed"));
    }
